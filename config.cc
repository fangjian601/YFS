...
bool config::ismember(std::string m) {
	bool r;
	assert(pthread_mutex_lock(&cfg_mutex)==0);
	r = isamember(m, mems);
	assert(pthread_mutex_unlock(&cfg_mutex)==0);
...
std::vector<std::string> config::get_curview() {
...
std::vector<std::string> config::get_prevview() {
	std::vector<std::string> v;
	assert(pthread_mutex_lock(&cfg_mutex)==0);
	v = get_view(myvid - 1);
	assert(pthread_mutex_unlock(&cfg_mutex)==0);
...
std::string config::print_curview() {
	std::string s;
	assert(pthread_mutex_lock(&cfg_mutex)==0);
...
bool config::add(std::string new_m) {
...
config::heartbeat_t config::doheartbeat(std::string m) {
	...
	if (ret != paxos_protocol::OK) {
		if (ret == rpc_const::atmostonce_failure || ret
				== rpc_const::oldsrv_failure) {
			mgr.delete_handle(m);
		} else {
...
bool config::paxos_run() {
	bool r = pro->run(myvid + 1, mems, value(mems));
	if (r) {
		printf("config::paxos_run: proposer returned success\n");
	} else {
		printf("config::paxos_run: proposer returned failure\n");
	}
	return r;
}
