...
void rsm_client::primary_failure() {
	...
	for (std::vector<std::string>::iterator it = known_mems.begin(); it
		...
		int ret = r.call(rsm_client_protocol::members, 0, temp_mems, rpcc::to(
				1000));
...
rsm_protocol::status rsm_client::invoke(int proc, std::string req,
	...
	while (1) {
		...
		primary.nref++;
		assert(pthread_mutex_unlock(&rsm_client_mutex)==0);
		ret = primary.cl->call(rsm_client_protocol::invoke, proc, req, rep,
				rpcc::to(5000));
		assert(pthread_mutex_lock(&rsm_client_mutex)==0);
		primary.nref--;
...
bool rsm_client::init_members(bool send_member_rpc) {
	if (send_member_rpc) {
