...
bool proposer::majority(const std::vector<std::string> &l1, const std::vector<
		std::string> &l2) {
	unsigned n = 0;
...
proposer::proposer(class paxos_change *_cfg, class acceptor *_acceptor,
		std::string _me) :
	cfg(_cfg), acc(_acceptor), me(_me), break1(false), break2(false), stable(
			true) {
...
bool proposer::prepare(unsigned instance, std::vector<std::string> &accepts,
	...
	printf("proposer:prepare: calculating my_n, before, my_n.n=%u, "
			"my_n.m=%s, acc->get_n_h().n=%u, me=%s\n",
			my_n.n, my_n.m.c_str(), acc->get_n_h().n, me.c_str());
	my_n.n = acc->get_n_h().n + 1 > my_n.n + 1 ? acc->get_n_h().n + 1 : my_n.n
			+ 1;
	my_n.m = me;
	printf("proposer:prepare: calculating my_n, after, my_n.n=%u, "
			"my_n.m=%s, acc->get_n_h().n=%u, me=%s\n",
			my_n.n, my_n.m.c_str(), acc->get_n_h().n, me.c_str());
		...
		if (h.get_rpcc()) {
			...
			if (h.get_rpcc()->call(paxos_protocol::preparereq, me, arg, res,
				...
				if (res.oldinstance) {
					...
					printf(
							"proposer::prepare: got prepareres from %s, res.n_a.n=%u, res.n_a.m=%s, res.v_a=%s\n",
							nodes[i].c_str(), res.n_a.n, res.n_a.m.c_str(),
							res.v_a.c_str());
					...
					if (res.v_a.size() != 0 && res.n_a > max_n_a) {
						max_n_a = res.n_a;
						v = res.v_a;
						printf("proposer::propose: updated value to newv=%s, "
								"max_n_a.n=%u, max_n_a.m=%s\n",
...
void proposer::accept(unsigned instance, std::vector<std::string> &accepts,
	...
	for (unsigned i = 0; i < nodes.size(); i++) {
		...
		if (h.get_rpcc()) {
			...
			if (h.get_rpcc()->call(paxos_protocol::acceptreq, me, arg, res,
					rpcc::to(1000)) == paxos_protocol::OK) {
...
void proposer::decide(unsigned instance, std::vector<std::string> nodes,
		std::string v) {
	for (unsigned i = 0; i < nodes.size(); i++) {
		handle h(nodes[i]);
		int res;
		...
		if (h.get_rpcc())
			h.get_rpcc()->call(paxos_protocol::decidereq, me, arg, res,
					rpcc::to(1000));
		else
...
paxos_protocol::status acceptor::preparereq(std::string src,
	...
	if (a.instance <= instance_h) {
		printf("acceptor::preparereq: responding with oldinstance to %s, "
		...
		printf("acceptor::preparereq: responding with prepareres to %s, "
				"a.n.n = %u, a.n.m = %s, n_h.n = %u, n_h.m = %s\n",
		...
		printf(" acceptor::preparereq: responding with reject to %s, "
				"a.n.n = %u, a.n.m = %s, n_h.n = %u, n_h.m = %s\n",
				src.c_str(), a.n.n, a.n.m.c_str(), n_h.n, n_h.m.c_str());
...
paxos_protocol::status acceptor::acceptreq(std::string src,
	...
	if (a.instance <= instance_h) {
		...
		printf("acceptor::acceptreq: responding with acceptres to %s, "
				"a.n.n = %u, a.n.m = %s, n_h.n = %u, n_h.m = %s\n",
				src.c_str(), a.n.n, a.n.m.c_str(), n_h.n, n_h.m.c_str());
		n_a = a.n;
		v_a = a.v;
		...
		printf(
				"acceptor::acceptreq: responding with reject to %s, a.n.n = %u, a.n.m = %s, n_h.n = %u, n_h.m = %s\n",
				src.c_str(), a.n.n, a.n.m.c_str(), n_h.n, n_h.m.c_str());
		r = 0;
	}
...
paxos_protocol::status acceptor::decidereq(std::string src,
	...
	if (a.instance <= instance_h) {
		printf("acceptor::decidereq: ignored oldinstance to %s, "
				"a.instance = %u, instance_h = %u\n",
				src.c_str(), a.instance, instance_h);
		; // ignore the old instance, since it won't matter
	} else {
		printf("acceptor::decidereq: committing value (decide from %s), "
				"a.instance = %u, a.v = %s\n",
